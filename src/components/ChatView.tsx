...
export default function ChatView({ chat, onCartChanged }: { chat?: Chat; onCartChanged?: () => void }) {
    ...
    <div className="space-y-4 p-4">
      {chat.messages.map((m) => (
        <div key={m.id} className={`max-w-4xl ${m.role === 'user' ? 'ml-auto' : ''}`}>
          ...
          {m.role === 'assistant' && m.result && <ResultCard result={m.result} sourceChatId={chat.id} sourceMessageId={m.id} onCartChanged={onCartChanged} />}
        </div>
      ))}
    </div>
  );
