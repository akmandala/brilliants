...
import ContentCartButton from './components/ContentCartButton';
import ContentCartDialog from './components/ContentCartDialog';
import { parserResultSchema } from './schemas';
import { clearContentCart, getContentCart, removeContentCartItem } from './storage/contentCartStore';
...
export default function App() {
  ...
  const [cartItems, setCartItems] = useState(getContentCart());
  const contentCartDownloadUrl = import.meta.env.VITE_CONTENT_CART_DOWNLOAD_URL || '/sample-content-cart.zip';
  ...
  return (
    <div className="flex h-screen">
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-end border-b border-zinc-300 p-2 dark:border-zinc-800">
          <ContentCartButton count={cartItems.length} onClick={() => setCartOpen(true)} />
        </div>
        <div className="min-h-0 flex-1 overflow-auto"><ChatView chat={selectedChat} onCartChanged={() => setCartItems(getContentCart())} /></div>
      ...
      <ContentCartDialog
        ...
        onRemove={(id) => setCartItems(removeContentCartItem(id))}
        onClear={() => setCartItems(clearContentCart())}
        downloadUrl={contentCartDownloadUrl}
      />
    </div>
  );
}
